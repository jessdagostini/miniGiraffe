...
int main(int argc, char *argv[]) {
    ...
    // time_utils_add(start, end, 1, omp_get_thread_num());
    ...
    /* Timer end */
    // end = omp_get_wtime();
    // time_utils_add(start, end, 2, omp_get_thread_num());
    
    // Create a cache variable for gbwt accesses
    // const CachedGBWTGraph* cache;
    ...
    for (int i = 0; i < size; i++) {
        ...
        for (seed_type seed : d.seeds) {
            ...
            match_initial(match, d.sequence, graph->get_sequence_view(seed.first));
            if (match.read_interval.first == 0) {
                match.left_full = true;
                match.left_maximal = true;
            }
            if (match.read_interval.second >= d.sequence.length()) {
            ...
            while (!extensions.empty()) {
                ...
                if (!curr.right_maximal) {
                    ...
                    graph->follow_paths(curr.state, false, [&](const gbwt::BidirectionalState& next_state) -> bool {
                        ...
                        size_t node_offset = match_forward(next, d.sequence, graph->get_sequence_view(handle), mismatch_limit);
                        if (node_offset == 0) { // Did not match anything.
                            return true;
                        }
                        next.path = get_path(curr.path, handle);
                        // Did the extension become right-maximal?
                        if (next.read_interval.second >= d.sequence.length()) {
                ...
                if (!curr.left_maximal) {
                    ...
                    graph->follow_paths(curr.state, true, [&](const gbwt::BidirectionalState& next_state) -> bool {
                        ...
                        match_backward(next, d.sequence, graph->get_sequence_view(handle), mismatch_limit);
            ...
            /* Timer end */
            // double end = omp_get_wtime();
            // time_utils_add(start, end, 4, omp_get_thread_num());
        }
        ...
        if (best_alignment < result.size() && result[best_alignment].internal_score <= MAX_MISMATCHES) {
            ...
            find_mismatches(d.sequence, *graph, result);
        }
