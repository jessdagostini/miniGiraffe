        ...
        for sched in scheduler:
            for opt in options:
                for rep in range(repetitions):
                    logger.info(f"[Main] Running test with batch size {batch}, threads {threads}, scheduler {sched}, and options {opt} on iteration {rep}")
                    output_csv = f"{batch}_{threads}_{sched}_{rep}_{opt}.csv"
